...
export default {
  ...
  // //------------------------传pdfData(包括time,id不一定,name,size)后端的交互
  //   name: 'PdfDetail',
  //   props: ['pdfData'], // 接收来自父组件的 pdfData 和 user 数据
  //   // 创建一个 JSON 对象
    
  //   created() {      
  //       // 发送请求到后端
  //       console.log('send user to pdf:', this.user.uid);
  //       console.log('send pdfData to pdf:', this.pdfData.file_name);
  //       axios.post('/api/send-pdf-data', { pdfData: this.pdfData, userId: this.user.uid })
  //           .then(response => {
  //               console.log('PDF data saved successfully:', response.data);
  //           })
  //           .catch(error => {
  ...
  methods: {
    ...
    // async fetchData() {
    //   try {
    //     const response = await axios.get('/api/get-pdf-data'); // 替换为你的后端接口路径
    //     this.receivedData = response.data;
    //     console.log('成功获取数据:', this.receivedData);
    //   } catch (error) {
